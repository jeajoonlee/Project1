...
public class ClientResDialog extends JDialog implements ActionListener, ItemListener{
	...
	public ClientResDialog(ClientRes rt, String index) {
		rtt = rt;
...
//		yv.addItemListener(this);					nv.addItemListener(this);
	...
	public void checkPan(ClientRes rt) {
		System.out.println("checkPan");
		row = rt.table.getSelectedRow();
		String st = rt.table.getValueAt(row, 3).toString().substring(0,1); 
		if(st.equals("1")) {
			priCheck(rt, row);
		} else {
			roomCheck(rt, row);
	...
	public void priCheck(ClientRes rt, int row) {
		System.out.println("priCheck");
		String sd = rt.table.getValueAt(row, 2).toString(); //예약일
		String sk = rt.table.getValueAt(row, 3).toString().substring(4,6); //p(오픈) or c(칸막이)//오픈형, 칸막이
		System.out.println(rt.table.getValueAt(row, 2).toString().substring(4,6));
		String sv = rt.table.getValueAt(row, 6).toString().substring(0,2); // 1일 1주 1개
		
		/*String sv = rt.table.getValueAt(row, 0).toString();
		int sv1 = sv.length();*/
		//String sv2 = sv.substring(sv1-2, sv1); //기간
		
		String ss = rt.table.getValueAt(row, 4).toString(); //지정 or 자유  
		...
		if(sk.equals("오픈형")) { //오픈형
			open.setSelected(true);
		} else {
			kan.setSelected(true);
		}
		
		if(sv.contentEquals("1일")) {
			vou.setSelectedIndex(1);
		} else if(sv.contentEquals("1주")) {
			vou.setSelectedIndex(2);
		} else if(sv.contentEquals("2주")) {
	...
	public void roomCheck(ClientRes rt, int row) {
		System.out.println("roomCheck");
		String sd = rt.table.getValueAt(row, 2).toString(); //예약일
		
		String sp = rt.table.getValueAt(row, 3).toString().substring(0,1); //인원 4,8
		/*int sp1 = sp.length();
		String sp2 = sp.substring(sp1-2, sp1-1); //인원*/
		
		String sv = rt.table.getValueAt(row, 5).toString();
        /*String sd = rt.table.getValueAt(row, 3).toString(); //예약일
	...
	public void actionPerformed(ActionEvent e) {
		dao = new ResDAO();
		id = (String) rtt.model.getValueAt(row, 0);
		System.out.println(row); 
		System.out.println(id);
		if(e.getSource() == pupdate) {
			System.out.println("pupdate");
			int ud = dao.clientpupdate(this, id);
			if (ud > 0) {
				
				messageBox(this, "수정 완료");
				dispose();
				dao.clientshowProcess(rtt.model);
				rtt.table.setModel(rtt.model);
				rtt.repaint();
			} else {
			...
			int ud = dao.clientrupdate(this, id);
			if (ud > 0) {
				messageBox(this, "수정 완료");
				dispose();
				dao.clientshowProcess(rtt.model);
